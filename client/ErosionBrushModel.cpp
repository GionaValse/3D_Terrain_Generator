...
ErosionBrushModel::ErosionBrushModel(std::string name, std::string res)
	: BrushModel(name, res, BrushToolType::EROSION),
	heightMap(nullptr)
{}

ErosionBrushModel::~ErosionBrushModel()
{}

void ErosionBrushModel::use(glm::vec3 coords, terrain::TerrainConfig config, std::vector<float>&image)
{
...
void ErosionBrushModel::setHeightMap(Eng::Texture * texture)
{
	heightMap = texture;
}
